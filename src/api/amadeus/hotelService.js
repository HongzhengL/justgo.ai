...
async function searchHotels({ cityCode, checkInDate, checkOutDate, adults = 1, filters = {} }) {
    console.log("Hotel search called with:", {
        cityCode,
        checkInDate,
        checkOutDate,
        adults,
    ...
    try {
        ...
        if (!hotelListRes.ok) {
            ...
            console.error(
                "Hotel List API Error:",
                hotelListRes.status,
                hotelListRes.statusText,
                errorText,
            );
            throw new Error(
                `Failed to fetch hotel list: ${hotelListRes.status} ${hotelListRes.statusText} - ${errorText}`,
            );
        }
        ...
        for (
            let i = 0;
            i < Math.min(20, hotelListData.data.length) &&
            workingHotels.length + hotelInfo.length < maxHotels;
            i++
        ) {
            ...
            try {
                ...
                if (!offersRes.ok) {
                    ...
                    if (hotelInfo.length < maxHotels - workingHotels.length) {
                        ...
                        console.log(
                            `Added ${hotel.name} to hotel info (no offers). Total info: ${hotelInfo.length}`,
                        );
                    }
                ...
                console.log(
                    `Successfully found offers for ${hotel.name}: ${offersData.data?.length || 0} offers`,
                );
                console.log(
                    `Hotel ${hotel.name} offers data structure:`,
                ...
                if (offersData.data && offersData.data.length > 0) {
                    ...
                    console.log(
                        `Added ${offersData.data.length} offers from ${hotel.name}. Total: ${workingHotels.length}`,
                    );

                    // If we have enough hotels, we can stop early
                    if (workingHotels.length + hotelInfo.length >= minHotels) {
                        console.log(
                            `Found minimum required hotels (${minHotels}), stopping search`,
                        );
                        break;
                    }
                } else {
                ...
                if (hotelInfo.length < maxHotels - workingHotels.length) {
                    ...
                    console.log(
                        `Added ${hotel.name} to hotel info (error). Total info: ${hotelInfo.length}`,
                    );
                }
                continue; // Try next hotel
            }
        ...
        const allHotels = [
            ...workingHotels,
            ...hotelInfo.slice(0, Math.max(0, minHotels - workingHotels.length)),
        ];

        // Return hotels - prioritize working hotels, then add hotel info to reach minimum
        if (allHotels.length > 0) {
            console.log(
                `Successfully collected ${workingHotels.length} working hotels and ${hotelInfo.length} hotel info`,
            );
