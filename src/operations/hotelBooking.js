...
export const bookHotelWithAI = async (bookingData, context) => {
    try {
        ...
        if (!bookingData.guestInfo || !bookingData.guestInfo.firstName || !bookingData.guestInfo.email) {
        ...
        const result = await Promise.race([
            ...
            new Promise((_, reject) => 
                setTimeout(() => reject(new Error('Automation timeout - using fallback booking')), 45000) // 45 seconds for real browser automation
            )
        ]);
...
export const getAIBookingStatus = async (args, context) => {
    try {
        ...
        try {
            ...
            console.warn('⚠️ Error getting agent status, using global status only:', agentError.message);
        ...
        const currentStatus = {
            ...
            progress: agentStatus.progress || Math.round((globalBookingStatus.currentStep / 5) * 100)
        };
