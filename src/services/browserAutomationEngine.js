...
export class BrowserAutomationEngine {
    ...
    async automateHotelBooking(hotelData, guestInfo, preferences, onProgress) {
        ...
        try {
            ...
            return {
                ...
                message:
                    "🎉 Real automation completed! Browser window is open with your booking in progress. Complete the payment to finish your reservation.",
            };
        } catch (error) {
    ...
    async selectRoom(page) {
        try {
            ...
            await page.waitForSelector('button[data-testid*="select-room"], .hprt-table button', {
                timeout: 5000,
            });

            const selectButton = await page.$(
                'button[data-testid*="select-room"], .hprt-table button',
            );
            if (selectButton) {
    ...
    async fillGuestInformation(page, guestInfo) {
        try {
            ...
            const formFields = [
                {
                    selectors: ['[name="firstname"]', '[name="guest_name"]'],
                    value: guestInfo.firstName,
                },
                {
                    selectors: ['[name="lastname"]', '[name="guest_surname"]'],
    ...
    selectOptimalBookingSite(hotelData) {
        ...
        if (
            hotelData.title?.toLowerCase().includes("booking.com") ||
    ...
    buildBookingDotComUrl(hotelData, guestInfo) {
        ...
        const hotelPath = "searchresults.html";
        ...
        try {
            return await page.evaluate(() => {
                ...
                const dates = document
                    .querySelector(".date-range, .booking-dates")
                    ?.textContent?.trim();
                const price = document
                    .querySelector(".total-price, .booking-total")
                    ?.textContent?.trim();
