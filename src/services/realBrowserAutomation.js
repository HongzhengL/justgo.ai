...
export class RealBrowserAutomation {
    ...
    async automateHotelBooking(hotelData, guestInfo, preferences, onProgress) {
        try {
            ...
            return {
                ...
                message: '🎉 ENHANCED automation completed! The browser shows your booking with all information pre-filled. Complete payment to finish your reservation!'
            };
    ...
    async selectSpecificDates(checkIn, checkOut) {
        try {
            ...
            const checkInElement = await this.page.waitForSelector(checkInSelector, { timeout: 5000 });
            if (checkInElement) {
            ...
            const checkOutElement = await this.page.waitForSelector(checkOutSelector, { timeout: 5000 });
            if (checkOutElement) {
            ...
            if (hotelName) {
                ...
                for (const cardSelector of cardSelectors) {
                    ...
                    for (const card of cards) {
                        ...
                        if (text && text.toLowerCase().includes(hotelName.toLowerCase().substring(0, 10))) {
    ...
    async selectRoomByTable() {
        const tableButtons = await this.page.$$('.hprt-table button, .roomstable button, table button');
