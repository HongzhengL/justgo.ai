...
export function FloatingCostSummary({ cards = [] }) {
    ...
    useEffect(() => {
            ...
            if (card.price?.amount) {
                ...
                if (amount > 0) {
                    if (card.type === 'flight') {
                        ...
                        const isReturn = title.includes('→') && (
                            title.includes('SFO → ORD') || 
                            title.includes('LAX → ORD') || 
                            title.includes('LIS → CDG') ||
                            title.includes('LIS → ORD') ||
                            title.match(/[A-Z]{3}\s*→\s*(ORD|JFK|LAX|CDG|LHR|BOS|SEA|DEN|ATL|MIA)/) // Common home airports
                        );
        ...
        breakdown.total = breakdown.flights.total + breakdown.hotels.total + 
                         breakdown.activities.total + breakdown.rentalCars.total;
                    ...
                    {costBreakdown.flights.count > 0 && (
                        <div className="cost-item">
                            <div className="cost-item-label">
                                ...
                                <span className="cost-item-count">({costBreakdown.flights.count})</span>
                            </div>
                            <div className="cost-item-amount">
                                {formatCurrency(costBreakdown.flights.total, costBreakdown.currency)}
                            </div>
                        </div>
                    )}
                    ...
                    {costBreakdown.hotels.count > 0 && (
                        <div className="cost-item">
                            <div className="cost-item-label">
                                ...
                                <span className="cost-item-count">({costBreakdown.hotels.count})</span>
                            </div>
                            <div className="cost-item-amount">
                    ...
                    {costBreakdown.activities.count > 0 && (
                        <div className="cost-item">
                            <div className="cost-item-label">
                                ...
                                <span className="cost-item-count">({costBreakdown.activities.count})</span>
                            </div>
                            <div className="cost-item-amount">
                                {formatCurrency(costBreakdown.activities.total, costBreakdown.currency)}
                            </div>
                        </div>
                    )}
                    ...
                    {costBreakdown.rentalCars.count > 0 && (
                        <div className="cost-item">
                            <div className="cost-item-label">
                                ...
                                <span className="cost-item-count">({costBreakdown.rentalCars.count})</span>
                            </div>
                            <div className="cost-item-amount">
                                {formatCurrency(costBreakdown.rentalCars.total, costBreakdown.currency)}
                            </div>
                        </div>
                    )}
                </div>
